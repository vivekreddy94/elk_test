...
def deployment_status(resource_type,service,total_pods) {
    boolean success = false
    counter = 0
    while (!success) {
        try{
            if(counter<30){
                ready_pods = sh(returnStdout: true, script: 'kubectl get ${resource_type} -n elk -l app=${service} | grep ${service} | awk \'{ print \$2 }\' | cut -d\'/\' -f 1').trim()
...
def filebeat_testing(){
    deployment_status("daemonset","filebeat","1")
